...
#Region "Bind Menus"
        Public Function getLeftmenu() As String
            Dim strMenu As String = ""
            Try
                Dim dataAccess As New DALDataAccess()
                Dim ds As New DataSet
                dataAccess.AddCommand(CommandType.StoredProcedure, "prc_getLeftNewMenu")
                dataAccess.AddParam("@compId", SqlDbType.NVarChar, ParamString(Me.CompanyID))
                dataAccess.AddParam("@indId", SqlDbType.NVarChar, ParamString(Me.IndustryID))
                strMenu = dataAccess.ExecuteScalar()
            Catch ex As Exception
                Utility.LogError(ex, "All Menu", True)
                Throw
            Finally
