...
Public Class import_library_data
    ...
    Private Sub lnkLibraryCSVTemplate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkLibraryCSVTemplate.Click
        Response.AppendHeader("Content-disposition", "attachment; filename=so-library-template.csv")
        Response.ContentType = "application/csv"
        Response.WriteFile(Server.MapPath("~") & ConfigurationManager.AppSettings("TemplatePath") & "so-library-template.csv")
        Response.End()
    End Sub
    ...
    Private Sub lnkCategoryHelpCSVTemplate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkCategoryHelpCSVTemplate.Click
        ...
        Response.End()
    End Sub
    Private Sub lnkCompanyHelpCSVTemplate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkCompanyHelpCSVTemplate.Click
        Response.AppendHeader("Content-disposition", "attachment; filename=csv-company-help-template.csv")
        Response.ContentType = "application/csv"
        Response.WriteFile(Server.MapPath("~") & ConfigurationManager.AppSettings("TemplatePath") & "csv-company-help-template.csv")
        Response.End()
    End Sub

    Private Sub lnkIndustryHelpCSVTemplate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkIndustryHelpCSVTemplate.Click
        Response.AppendHeader("Content-disposition", "attachment; filename=csv-industry-help-template.csv")
        Response.ContentType = "application/csv"
        Response.WriteFile(Server.MapPath("~") & ConfigurationManager.AppSettings("TemplatePath") & "csv-industry-help-template.csv")
        Response.End()
    End Sub
...
#Region "import CSV Customer Data"
    ...
    Sub ImportLibraries(ByRef strErrorRows As String, ByRef ds As DataSet)
        ...
        For Each row As DataRow In ds.Tables(0).Rows
            If row("OldText").ToString().Trim.Length > 0 Or row("NewText").ToString().Trim.Length > 0 Then
                Dim objConn As New DALDataAccess()
                ' Try
                objConn.AddCommand(CommandType.StoredProcedure, "prc_ImportLibraryCSVData")
                objConn.AddParam("@industry", SqlDbType.VarChar, row("Industry").ToString().Trim())
                objConn.AddParam("@company", SqlDbType.VarChar, row("Company").ToString().Trim())
                objConn.AddParam("@OldText", SqlDbType.VarChar, row("OldText").ToString().Trim())
                objConn.AddParam("@newText", SqlDbType.VarChar, row("NewText").ToString().Trim())
                objConn.AddParam("@Category", SqlDbType.VarChar, row("Category").ToString().Trim())
                objConn.AddParam("@LibraryLink", SqlDbType.VarChar, row("Link").ToString().Trim())
                objConn.AddParam("@LibraryImage", SqlDbType.VarChar, row("Image").ToString().Trim())
