...
Public Class WPageHandler
    ...
    Public Function GetHttpHandler(ByVal requestContext As RequestContext) As System.Web.IHttpHandler Implements IRouteHandler.GetHttpHandler
        ...
        Try
            ...
            'ds.ReadXml("\\ace\Projects\sandiegohomeservice.com\Project\x301Redirect.xml")
            'online
            'ds.ReadXml("C:\inetpub\vhosts\sandiegohomeservice.com\httpdocs\x301Redirect.xml")
            ...
            End If
            'offline
            'ds.ReadXml("\\ace\Projects\sandiegohomeservice.com\Project\x302Redirect.xml")
            'online
            'ds.ReadXml("C:\inetpub\vhosts\sandiegohomeservice.com\httpdocs\x302Redirect.xml")
            'If ds.Tables.Count > 0 Then
            '    Dim strCurrentUrl As String = HttpContext.Current.Request.Url.ToString()
            '    For Each dr As DataRow In ds.Tables(0).Rows
            '        If strCurrentUrl = dr.Item("r_oldurl") Then
            '            HttpContext.Current.Response.Status = "302 Moved Permanently"
            '            HttpContext.Current.Response.AddHeader("Location", "http://www.sandiegohomeservice.com")
            '            HttpContext.Current.Response.Redirect("http://www.sandiegohomeservice.com")
            '            Exit Function
            '        End If
            '    Next
            'End If
