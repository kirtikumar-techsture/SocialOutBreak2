...
Public Class customize_sweepstake
...
#Region "Bind Tab"
    Private Sub BindSweepstake()
        Try
            ...
            ds = objBAL.GetSweepstakeTemplate
            If ds.Tables(0).Rows.Count > 0 Then
                txtname.Text = ds.Tables(0).Rows(0).Item("name")
                divSweepstakeHtml.InnerHtml = ds.Tables(0).Rows(0).Item("content")
                'divLikeHtml.InnerHtml = ds.Tables(0).Rows(0).Item("content")
                'divUnLikeHtml.InnerHtml = ds.Tables(0).Rows(0).Item("content").ToString() '.Replace("rm-css-text", "rm-css-text11").Replace("rm-css-bg", "rm-css-bg11")
                ViewState("ExpressTabId") = CInt(ds.Tables(0).Rows(0).Item("libId"))
                ...
                For Each item As DataListItem In dstFanPages.Items
                    ...
                    If myCheckBox.Checked = True Then
                        ...
                        If strPageId <> "" AndAlso strPageAccessToken <> "" Then
                            ...
                            Dim obj As New BALCustomTab
                            'obj.Customise = CInt(ViewState("SavedTabId"))
                            ...
                            If (CInt(ViewState("SavedTabId"))) = 0 Then
                                obj.swid = CInt(Request.QueryString("swId"))
                            Else
                                obj.swid = (CInt(ViewState("SavedTabId")))
                            End If
                            ...
                            obj.CustomTabContent = hdnLike.Value
...
#Region "Save"
    ...
    Private Sub SaveTab(ByVal intSaveType As Int16)
        ...
        '        objBAL.Customise = CInt(ViewState("SavedTabId"))
        '        objBAL.swid = CInt(Request.QueryString("swid"))
        '        objBAL.LikeUnlike = 0 'hdnLike.Value
        '        objBAL.UserId = CInt(Session("SiteUserId"))
        '        objBAL.CustomTabName = txtCustomTabName.Value
        '        objBAL.CustomTabContent = hdnLike.Value
        ...
        '        objBAL.ShareEmailText = txtShareEmailText.Text
        '        objBAL.getSwType = ViewState("sw_type")
        ...
        '        'objBAL.ShareImage = file_name_s
        '        'objBAL.SharePDF = file_name_pdf

        '        ViewState("SavedTabId") = CInt(objBAL.SaveSweepstakeContent())

        '        BindSaveSweepstakeByID(CInt(ViewState("SavedTabId")))
        '        'BindSaveSweepstakeByID(temp)
        '        Dim siteUrl As String = ConfigurationManager.AppSettings("AppPath") & "generate-sw-image.aspx?id=" & Convert.ToInt32(ViewState("SavedTabId"))
        ...
        ViewState("SavedTabId") = CInt(objBAL.SaveCustomiseSweepstakeContent())

        BindSaveSweepstakeByID(Convert.ToInt32(ViewState("SavedTabId")))
        'BindSaveSweepstakeByID(temp)
        Dim siteUrl As String = ConfigurationManager.AppSettings("AppPath") & "generate-sw-image.aspx?id=" & Convert.ToInt32(ViewState("SavedTabId"))
        Dim browserWidth As Integer = Convert.ToInt32(800)
        Dim browserHeight As Integer = Convert.ToInt32(600)
...
#Region "Redirect"
    ...
    Private Sub Redirect()
        If Convert.ToString(ViewState("SavedTabId")) <> "" Then
            Response.Redirect("selected-sweepstake.aspx?swid=" & ViewState("SavedTabId") & "&sid=" & hdnSaveMenu.Value)
        Else
...
#End Region
    ...
    'Private Sub lnkSaveUnlike_ServerClick(sender As Object, e As System.EventArgs) Handles lnkSaveUnlike.ServerClick
    '    SaveTab(5)
    'End Sub
