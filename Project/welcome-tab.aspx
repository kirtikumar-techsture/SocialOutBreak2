<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="welcome-tab.aspx.vb" Inherits="tsma.welcome_tab" %>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        FB.Canvas.setSize();
     /* alert("before" + $('#lnkSubmit').attr('onclick'));
	  $('#lnkSubmit').attr('onclick','return submitcontact();');			
	  alert("after" + $('#lnkSubmit').attr('onclick'));*/
	   /* $("a[rel^=lnkSubmit]").each(
					function(){
						$(this).attr("onclick","return submitcontact();");			
					}
				);*/
		...
		/*alert('123');
		var urlvideo1= $("div[rel^=video1] a:first").attr('rel');
		alert(urlvideo1);*/
		//alert($('#videoMain').html());
...
//				var msgval = $("#msg").val().replace(regx,"<br/>");
            ...
            //$.ajax({ url: '/ajax/ajax-custom-tab-info.aspx?nm=' + $("#name").val() + '&appid=' +  $("#hdnAppId").val()  + '&appname=' + $("#hdnAppName").val() + '&msg=' + $("#msg").val()  + '&em=' + $("#email").val() + '&lnm=' + $("#name").val() + '&ph=' + $("#phno").val() + '&fbuid=' + $("#hdnFBUserId").val() + '&tsmauid=' + $("#hdnTSMAUserId").val() + '&fbpid=' + $("#hdnFBPageId").val() + '&fbpagename=' + $("#hdnFBPageName").val(), success: function (result) {
			$.ajax({ url: '/ajax/ajax-custom-tab-info.aspx?nm=' + $("#name").val() + '&appid=' +  $("#hdnAppId").val()  + '&appname=' + $("#hdnAppName").val() + '&msg=' + $("#msg").val()  + '&em=' + $("#email").val() + '&lnm=' + $("#name").val() + '&ph=' + $("#phno").val() + '&fbuid=' + $("#hdnFBUserId").val() + '&tsmauid=' + $("#hdnTSMAUserId").val() + '&fbpid=' + $("#hdnFBPageId").val() + '&fbpagename=' + $("#hdnFBPageName").val(), success: function (result) {
...
<script type="text/javascript">
    function share(message) {
        ...
        picture: 'http://so.tsmaapplication.com/Content/images/we_build_img.jpg',
        caption: 'TSMA-Social Outbreak',
        description: 'You can share your reviews on Facebook',
        message: ''
        });
       ...
       /*  FB.ui(
        ...
        picture: 'https://www.uswebsocial.com/Content/images/we_build_img.jpg',
...
<script type="text/javascript" charset="utf-8">
   ...
   <div id="divContent" style="width:520px;margin-left:auto; margin-right:auto; text-align:center;" align="center" runat="server"></div>
    </form>
...
<script type="text/javascript">
    //$(".hide").css("display","block");
		var vidcont = $('#videoMain').html();
		function ShowMainVideo(bbj)
		{
			$("#videoMain").html(vidcont);
			return false;
		}
		...
		if (urlmain == null | urlmain == '') {
			$("div[rel^=videoMain] a:first").attr('href', 'javascript:;');
		}
		else
		{
			...
			if (obj != '') {
				...
				$("#videoMain").html('<iframe width="' + vwidth +'" height="' + vheight +'" src="https://www.youtube.com/embed/' + vid + '?feature=player_detailpage" frameborder="0" allowfullscreen></iframe>');
			}
		}
	    ...
	    $("div[rel^=video1] a:first").click(function()
		{
			...
			if (url1 == null | url1 == '') {
				...
				if (obj != '') {
					...
					res = (results === null) ? "" : results[1];
					vid = res.replace("http://youtu.be/", "");
					$("#videoMain").html('<div><div style="flot:left;"><iframe width="' + vwidth +'" height="' + vheight +'" src="https://www.youtube.com/embed/' + vid + '?feature=player_detailpage&autoplay=1" frameborder="0" allowfullscreen></iframe></div><div style="float:right; padding-top:2px; padding-left:2px; width:'+ vwidth +'; position:absolute; color:#FFFFFF; font-size:12px; z-index:10000"><a href="javascript" onclick="ShowMainVideo();"><img src="https://www.summasocialapp.com/content/images/vid_close.png" border="0" align="right" /></a></div></div>');
				}
			}
		...
		$("div[rel^=video2] a:first").click(function()
		{
			var url1 = $("div[rel^=video2] a:first").attr('rel');
			if (url1 == null | url1 == '') {
				$("div[rel^=video2] a:first").attr('href', 'javascript:;');
			}
			else {
				$("div[rel^=video2] a:first").attr('href', url1);
				var vwidth = $("#videoMain").css('width');
				var vheight = $("#videoMain").css('height');
				var obj = url1;
				$("#videoMain").html('');
		 ...
		 $("div[rel^=video3] a:first").click(function()
		{
			var url1 = $("div[rel^=video3] a:first").attr('rel');
			if (url1 == null | url1 == '') {
				$("div[rel^=video3] a:first").attr('href', 'javascript:;');
			}
			else {
				$("div[rel^=video3] a:first").attr('href', url1);
				var vwidth = $("#videoMain").css('width');
				var vheight = $("#videoMain").css('height');
				var obj = url1;
				$("#videoMain").html('');
