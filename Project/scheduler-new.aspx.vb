    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            If Not Page.IsPostBack Then
                ...
                Dim url1 As String = "https://graph.facebook.com/me?fields=id,name,link,picture&return_ssl_resources=true&access_token={0}"
        ...
        '    lblFanPageMessage.Text = "Facebook is experiencing problems at this time. Please save your work and try to publish later."
        'End Try
    End Sub
    '#End Region
    ...
    Sub BindDraftData(ByVal id As Integer)
        ...
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows(0).Item("sm_ScheduleTimeZone") <> "" Then
    ...
    Sub DeleteByID(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                ...
                ElseIf CInt(sender.CommandArgument = 4) Then
                    hdnSetTab.Value = 4
...
#Region "Custom Paging Drafts"
    ...
    Sub GenreateControls()
        ...
        If (ViewState("TotalPage") > 1) Then
            ...
            '        ''''''------------------''''''
            '        objlink = New LinkButton()
            '        objlink.ID = "Drafts" & i + ViewState("TotalRecords")
            '        objlink.Text = i
            '        AddHandler objlink.Click, AddressOf lnkPage_click
            '        phPaging1.Controls.Add(objlink)
            '        phPaging1.Controls.Add(New LiteralControl("&nbsp;&nbsp;|&nbsp;&nbsp;"))
            ...
            '        ''''''------------------''''''
            '        objlabel = New Label()
            '        objlabel.ID = "Drafts" & i + ViewState("TotalRecords")
            '        objlabel.Text = i
    ...
    Private Sub lnkPage_click(ByVal sender As Object, ByVal e As System.EventArgs)
        'If (CInt(CType(sender, LinkButton).ID.Replace("Drafts", "")) > ViewState("TotalRecords")) Then
        '    ViewState("CurrentPageIndex") = CType(sender, LinkButton).ID.Replace("Drafts", "") - ViewState("TotalRecords")
        'Else
        '    ViewState("CurrentPageIndex") = CType(sender, LinkButton).ID.Replace("Drafts", "")
        'End If
...
#Region "Custom Paging Scheduled Posts"
    ...
    Sub GenreateControlsScheduled()
        ...
        If (ViewState("TotalPageScheduled") > 1) Then
            ...
            '        ''''''------------------''''''
            '        objlink = New LinkButton()
            '        objlink.ID = "scheduled" & i + ViewState("TotalRecordsScheduled")
            '        objlink.Text = i
            '        AddHandler objlink.Click, AddressOf lnkPageScheduled_click
            '        phPagingScheduled.Controls.Add(objlink)
            '        phPagingScheduled.Controls.Add(New LiteralControl("&nbsp;&nbsp;|&nbsp;&nbsp;"))
            ...
            '        ''''''------------------''''''
            '        objlabel = New Label()
            '        objlabel.ID = "Scheduled" & i + ViewState("TotalRecordsScheduled")
            '        objlabel.Text = i
            '        objlabel.CssClass = "curpage"
    ...
    Sub SaveToMyLibrary(ByVal sender As Object, ByVal e As CommandEventArgs)
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                ...
                BindLibraryCategories()
                ClearData()
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, ";ShowHide;", ";ShowUserLib(" & intCat & ");SaveScheduleAlert('library message is Saved Successfully');", True)
            Else
                Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
            End If
        Catch ex As Exception
    ...
    Private Sub lnkSaveDraft_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkSaveDraft.ServerClick
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                ...
                Dim strmessage As String = txtMessage.Value.Replace("<br>", Chr(10))
                ...
                If RequestContext.RouteData.Values("sm_Id") <> "" Then
                    ...
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "SaveAlert", "SaveScheduleAlert('Draft updated successfully');", True)
                Else
                    Dim objBAL As New BALSchedulePost
                    objBAL.TSMAUserId = Session("SiteUserId")
                    objBAL.FBUserId = Session("FacebookUserId")
...
#Region "Common function for posting data on Facebook "
    Function PostData(ByVal intTaskId As Integer)
        'Try
        Dim AccessToken As String = Session("FacebookAccessToken")
        Dim objBAL As New BALSchedulePost
        ...
        'dtAutoPostDate = Convert.ToDateTime(txtActivationDate.Value).ToString("MM/dd/yyyy") & " " & selActivationHour.Value & ":" & selActivationMinute.Value & selAMPM.Value  'Date.Now.ToString()

        'dtAutoPostDate = If((Convert.ToString(dtAutoPostDate) <> ""), dtAutoPostDate, Date.Now)
                ...
                Try
                    ...
                    ElseIf intTaskId = 2 Then
                        If RequestContext.RouteData.Values("sm_Id") <> "" Then
                            ...
                            objBAL.PostType = 1
                            objBAL.UpdateDrafts()
                            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "SaveAlert", "SaveScheduleAlert('Message Updated successfully');", True)
                        Else
                            objBAL.IsPosted = 0
                            objBAL.PostType = 1
                            ds = objBAL.AddQuickPostMaster()
