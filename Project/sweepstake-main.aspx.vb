...
Public Class sweepstake_main
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            ...
            Dim strpage As String = CStr(HttpContext.Current.Request.Url.AbsolutePath) & "?id=" & Request.QueryString("id")
            Dim obj As New BALCustomTab
            Dim ds1 As New DataSet
            obj.Page = CStr(HttpContext.Current.Request.Url.AbsolutePath) & "?id=" & Request.QueryString("id")
            'Response.Write(obj.Page)
            ...
            'objBALimage.PageId = Convert.ToInt32(Request("id"))
            dsimage = objBALimage.GetSweepstakeByCompOrIndustry
            ...
            If ds1.Tables(0).Rows.Count > 0 Then
                If strpage = ds1.Tables(0).Rows(0).Item("vt_Page").ToString Then
    ...
    Private Sub bind_mySweeptakeTabs(ByVal direction As Integer)
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            If Not Session("SiteUserId") Is Nothing Then
                ...
                obj.CategoryId = Request.QueryString("id")
                'Response.Write(obj.UserId & "//" & obj.FBUserId & "//" & obj.FBPageId & "//" & obj.FBUserEmail & "//" & obj.Cid)
                ' Response.End()
                ds = obj.GetMySweepstakeTabsNew()
                If ds.Tables(0).Rows.Count > 0 Then
                    If ds.Tables(0).Rows(0).Item("sw_Image") = "" Then
                        imgMyCustomTab.Src = "/content/images/nomysidebar.jpg"
                    Else
                        ltrName.Text = ds.Tables(0).Rows(0).Item("cs_Name").ToString
                        imgMyCustomTab.Src = "/uploads/" & ds.Tables(0).Rows(0).Item("sw_Image")
                        hdnImageName.Value = ds.Tables(0).Rows(0).Item("sw_Image")
                    End If
                    ViewState("cs_FBPageId") = ds.Tables(0).Rows(0).Item("sw_FBPageId")
                    ViewState("cs_FBPageAccessToken") = ds.Tables(0).Rows(0).Item("sw_FBPageAccessToken")
                    hdnCustomTab.Value = ds.Tables(0).Rows(0).Item("cs_Id")
                    hdnMasterId.Value = ds.Tables(0).Rows(0).Item("cs_Id")
                    ViewState("cs_IsPublished") = ds.Tables(0).Rows(0).Item("cs_IsPublished")
                Else
    ...
    Private Sub ibtnNext_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ibtnNext.Click
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            bind_mySweeptakeTabs(1)
        Else
            Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
        End If
    End Sub

    Private Sub ibtnPre_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ibtnPre.Click
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            bind_mySweeptakeTabs(0)
        Else
            Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
    ...
    Private Sub lnlPublish_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnlPublish.Click
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            If hdnImageName.Value <> "0" Then
                Response.Redirect("selected-tab.aspx?id=-" & hdnCustomTab.Value)
            End If
        Else
            Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
        End If
    ...
    Private Sub lnkdelete1_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkdelete1.ServerClick
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            Dim obj As New BALCustomTab
            'If Not IsDBNull(ViewState("cs_IsPublished")) AndAlso ViewState("cs_IsPublished") = 1 Then
            '    If ViewState("cs_FBPageId") <> "" And ViewState("cs_FBPageAccessToken") <> "" Then
            ...
            '        Dim fbRequest As WebRequest = WebRequest.Create(String.Format(url1, ViewState("cs_FBPageId"), ViewState("cs_FBPageAccessToken"), AppID))
            ...
            '            strPageId = ViewState("cs_FBPageId") '"103823149672939"
            '            Dim strPageAccessToken As String = ViewState("cs_FBPageAccessToken") '"AAAD2ZCc3HZAxABAIDxzyAwOqZCgzSRFVkLHqiLl88muU7QIRYTE1ZBEnPM7GTL7yZCbs7REZA0cVul1DsSWs2ntOKZB4boRTDPDZB8htHZBcn3C5Jlnlnqw4R"
            ...
            Dim ds As New DataSet
            obj.UserId = Session("SiteUserId")
            obj.swid = hdnCustomTab.Value
            obj.DeleteMySweepstakes()
    ...
    Private Sub lnkCopy_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkCopy.ServerClick
        If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
            Dim ds As New DataSet
            Dim obj As New BALSweepStake
            obj.TSMAUserId = Session("SiteUserId")
            obj.FBUserId = Session("FacebookUserId")
            obj.swid = hdnCustomTab.Value
            ds = obj.CopyMySavedSweepstake()
            If ds.Tables(0).Rows.Count > 0 Then
                'Response.Redirect("custom-tabs.aspx?id=" & ds.Tables(0).Rows(0).Item("CopyId"))
                Response.Redirect("selected-sweepstake.aspx?swid=" & ds.Tables(0).Rows(0).Item("CopyId"))
            End If
        Else
            Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
        End If
